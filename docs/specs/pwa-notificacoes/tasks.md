...
- [x] **1.1** `app/icons/[size]/route.tsx`: ImageResponse (quadrado verde + carrinho) em 192/512. Fallback de marca.
- [x] **1.2** `app/apple-icon.tsx`: ícone iOS (180). `app/manifest.ts`: name/short_name/start_url/standalone/theme/icons (192,512,512 maskable). → RF6. `T:` build; GET /manifest.webmanifest e /icons/512 respondem.

## Escopo 2 — useOffline + banner
- [x] **2.1** `next.config.ts`: `experimental.useOffline = true`. → ADR-1/RF7.
- [x] **2.2** `OfflineBanner` (client, `next/offline`) no layout raiz. → RF8. Rótulo offline em ações-chave (RF9) onde couber.

## Escopo 3 — Service worker mínimo (cold-start + update)
- [x] **3.1** `public/sw.js`: cache versionado `mm-shell-v<N>`; navigate→network-first (fallback shell/`/offline`); estáticos hasheados→cache-first; `/api/*`+RSC→network-only; activate limpa caches antigos; skipWaiting só via postMessage. → RF11/RF12/RF13.
